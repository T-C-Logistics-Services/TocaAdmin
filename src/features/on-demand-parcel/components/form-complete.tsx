...
export default function FormComplete({ booking }: FormCompleteProps) {
  ...
  const formatTime = (timeString: string) => {
    ...
    if (timeString.includes(':')) {
      ...
      const minutesPart = parts[1].split(/\s+/)[0]
      minutes = parseInt(minutesPart, 10)

      // Check if PM is in the string and adjust hours accordingly
      if (timeString.toLowerCase().includes('pm') && hours < 12) {
        hours += 12
      }
      // Check if AM is in the string and it's 12 AM
      if (timeString.toLowerCase().includes('am') && hours === 12) {
        hours = 0
      }
    } else {
      // Try to parse as a timestamp or other format
    ...
    <div id='print-area'>
      <div className='text-center'>
              ...
              <tr className=''>
                ...
                <td className='py-2 text-right text-sm text-gray-600'>
                  <p className='text-base font-semibold text-black'>
                    {booking.package.vehicle}
                  </p>
                  <p className=''>
                    Pick-up Time: {formatTime(booking.order.pickUpTime || '')}
                  </p>
                  <p className=''>
                    Delivery Date:{' '}
                    {formatDate(booking.order.deliveryDate || '')}
                  </p>
                </td>
              </tr>
              <tr className=''>
        ...
        <table className='mt-2 w-full border-collapse border-spacing-0 border border-gray-200'>
          <tbody>
            <tr className='border-t border-b border-gray-200'>
              <td rowSpan={4} className='border border-gray-200' align='center'>
                <div
        ...
        <div className='mt-8 text-center print:hidden'>
          <button
            className='bg-yellow-500 px-6 py-3 text-lg font-semibold text-white shadow-md'
